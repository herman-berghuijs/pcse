...
import yaml
from sqlalchemy import MetaData, select, Table, and_, text
...
def fetch_crop_name(engine, idcrop_parametrization):
    ...
    table_crop = Table("l_crop_parametrizations", metadata, autoload=True)
...
class WeatherObsGridDataProvider(WeatherDataProvider):
    ...
    def __init__(self, engine, idgrid, start_date=None, end_date=None, use_cache=True,
                 recalc_ET=False, recalc_TEMP=False, table_name='l_weather_obs_grid'):
        ...
        if not self._self_load_cache(self.idgrid) or self.use_cache is False:
            ...
            line1 = "Weather data retrieved from CGMS 23 db %s" % str(engine)[7:-1]
    ...
    def _fetch_location_from_db(self, metadata):
        ...
        tg = Table("l_grids", metadata, autoload=True)
        ...
        # Report success
        msg = "Successfully retrieved location information from 'grids' table for grid %s"
...
class AgroManagementDataProvider(list):
    ...
    def __init__(self, engine, idgrid, idcrop_parametrization, campaign_year, campaign_start=None):
        ...
        metadata = MetaData(engine)
        t = Table("l_crop_calendars", metadata, autoload=True)
        sm = select([t], and_(t.c.idgrid == self.idgrid,
                              t.c.idcrop_parametrization == self.idcrop,
                              t.c.idseason == self.campaign_year))
        ...
        # Start of the crop
        if row.start_period.upper() == "SOWING":
            self.agro["crop_start_date"] = check_date(row.start_date)
            self.agro["start_type"] = "sowing"
        elif row.start_period.upper() == "EMERGENCE":
            self.agro["crop_start_date"] = check_date(row.start_date)
        ...
        if row.end_period.upper() == "MATURITY":
            self.agro["crop_end_date"] = ""
            self.agro["end_type"] = "maturity"
            self.agro["max_duration"] = (check_date(row.end_date) - self.agro["crop_start_date"]).days
            self.end_date = check_date(row.end_date)
        elif row.end_period.upper() == "HARVESTING":
            self.agro["crop_end_date"] = check_date(row.end_date)
...
class SoilDataProviderSingleLayer(dict):
    ...
    def __init__(self, engine, idstu):
        ...
        msg = f"{self.__class__} implemented for CGMS23EUR yet!"
...
class CropDataProvider(dict):
    ...
    def __init__(self, engine, idgrid, idcrop_parametrization):
        ...
        self.idvariety = self._get_crop_variety(engine, self.idgrid, self.idcrop_parametrization)

        # get the parameters from the CGMS db
        self._fetch_crop_parameter_values(engine, self.idcrop_parametrization)
        self._fetch_variety_parameter_values(engine, self.idcrop_parametrization, self.idvariety)
        self.update(self.parameters_additional)

        # Finally add crop name
        self["CRPNAM"] = self.crop_name

        # specific management of IDSL parameter
        self["IDSL"] = 0
        if  "USEPHOTOPER" in self and self["USEPHOTOPER"]==1:
            self["IDSL"] = 1
        if  "USEVERNALISATION" in self and self["USEVERNALISATION"]==1:
            self["IDSL"] = 2

    def _get_crop_variety(self, engine, idgrid, idcrop_parametrization):
        """Derives variety ID for given crop and idgrid
        """
        metadata = MetaData(engine)
        t = Table('l_crop_spatializations', metadata, autoload=True)
        s = select([t.c.idvariety],
                   and_(t.c.idgrid == idgrid,
                        t.c.idcrop_parametrization == idcrop_parametrization))
        with engine.connect() as DBconn:
            row = DBconn.execute(s).fetchone()
        ...
        return int(row.idvariety)

    def _fetch_crop_parameter_values(self, engine, idcrop_parametrization):
        ...
        # Pull single value parameters from DB
        meta = MetaData(engine)
        tbl = Table("L_CROP_PARAMETERS", meta, autoload=True)
        with engine.connect() as DBconn:
            for parameter_code in self.parameter_codes_single:
                if parameter_code == "IDSL": # skip IDSL parameter
                    continue
                # sql = text(f"SELECT xvalue FROM L_CROP_PARAMETERS where idcrop_parametrization={idcrop_parametrization} and crop_parameter='{parameter_code}'")
                s = select([tbl.c.xvalue], and_(tbl.c.idcrop_parametrization==idcrop_parametrization, tbl.c.crop_parameter==parameter_code))
                row = DBconn.execute(s).fetchone()
                if not row:
                    msg = f"Parameter value expected for idcrop_parametrization={idcrop_parametrization} and parameter {parameter_code}"
                    raise exc.PCSEError(msg)

                if parameter_code not in self.single2tabular:
                    self[parameter_code] = float(row.xvalue)
                else:
        ...
        # Pull tabular parameters from CROP_PARAMETER_VALUE
            for parameter_code in self.parameter_codes_tabular:
                ...
                # sql = text(f"SELECT xvalue, yvalue FROM L_CROP_PARAMETERS WHERE "
                #            f"idcrop_parametrization={idcrop_parametrization} and crop_parameter like '{pattern}'")
                s = select([tbl.c.xvalue, tbl.c.yvalue],
                           and_(tbl.c.idcrop_parametrization == idcrop_parametrization,
                                tbl.c.crop_parameter.like(pattern)))
                rows = DBconn.execute(s).fetchall()

                if len(rows) == 0:
                    msg = ("No parameter value found for idcrop_parametrization=%s, parameter_code='%s' while "
                           "tabular parameter expected." % (idcrop_parametrization, parameter_code))
                    raise Exception(msg)
                if len(rows) == 1:
                    msg = ("Single parameter value found for idcrop_parametrization=%s, parameter_code='%s' while "
                           "tabular parameter expected." % (idcrop_parametrization, parameter_code))
                    raise Exception(msg)
                values = []
                for row in rows:
    ...
    def _fetch_variety_parameter_values(self, engine, idcrop_parametrization, idvariety):
        """Derived the crop parameter values from the table l_crop_variety_parameters
        for given idcrop_parametrization, idvariety and add directly to dict self[].
        """

        meta = MetaData(engine)
        tbl = Table("L_CROP_VARIETY_PARAMETERS", meta, autoload=True)
        with engine.connect() as DBconn:
            for parameter_code in self.parameter_codes_single:
                if parameter_code == "IDSL": # skip IDSL parameter
                    continue

                s = select([tbl.c.xvalue], and_(tbl.c.idcrop_parametrization==idcrop_parametrization,
                                                tbl.c.crop_parameter==parameter_code,
                                                tbl.c.idvariety==idvariety))
                row = DBconn.execute(s).fetchone()
                if row:
                    if parameter_code not in self.single2tabular:
                        self[parameter_code] = float(row.xvalue)
                    else:
                        code, value = self._convert_single2tabular(parameter_code, float(row.xvalue))
                        self[code] = value

            # Pull tabular parameters
            for crop_parameter in self.parameter_codes_tabular:
                pattern = crop_parameter + r'%'
                s = select([tbl.c.xvalue, tbl.c.yvalue],
                           and_(tbl.c.idcrop_parametrization == idcrop_parametrization,
